...
void LinkedList<T>::insertOrdered(const T& newData) {
     ...
     //Initializing a new node on the heap
	...
	while(trav->next != nullptr && trav->next->data < new_node->data){
	    trav = trav->next;
	}
...
LinkedList<T> LinkedList<T>::merge(const LinkedList<T>& other) const {
  ...
  LinkedList<T> merged;
    ...
    Node* travL = left.getHeadPtr();
    Node* travR = right.getHeadPtr();


    while(!left.empty() && !right.empty()){

	if (travL->data > travR->data){
	    merged.pushBack(travR->data);
	    travR = travR->next;
	    right.popFront();
	}
	else{
	    merged.pushBack(travL->data);
	    travL = travL->next;
	    left.popFront(); 
	}
